...
// */
SkeletonShot::~SkeletonShot(void)
{
...
bool SkeletonShot::Init(void)
{
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Image/SP3Images/Enemies/Attacks/bone.png", true);
	if (iTextureID == 0)
	{
		std::cout << "Failed to load SkeletonShot tile texture" << std::endl;
		return false;
	}
 ...
 */
void SkeletonShot::Update(const double dElapsedTime)
{
	if (!bIsActive)
		return;
...
bool SkeletonShot::InteractWithPlayer()
{
